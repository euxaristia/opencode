...
import { repositoryCacheRoot } from "../../src/util/repository"
  ...
  it.live("clones a repo into the managed cache and reuses it on subsequent calls", () =>
    provideTmpdirInstance((_dir) =>
      Effect.gen(function* () {
        ...
        expect(cloned.metadata.localPath).toBe(path.join(repositoryCacheRoot(), "github.com", "owner", "repo"))
  ...
  it.live("clones generic git URLs into the managed cache", () =>
    provideTmpdirInstance((_dir) =>
      Effect.gen(function* () {
        ...
        expect(result.metadata.localPath.startsWith(path.join(repositoryCacheRoot(), "file"))).toBe(true)
